...
import URLT.Monad (URLT, ShowURL(showURL), URL)

instance (Functor m, Monad m) => HSX.XMLGen (URLT url m) where
...
instance (Functor m, Monad m) => HSX.EmbedAsAttr (URLT url m) Attribute where
    asAttr = return . (:[]) . UAttr 

instance (Functor m, Monad m) => HSX.EmbedAsAttr (URLT url m) (Attr String Char) where
    asAttr (n := c)  = asAttr (n := [c])

instance (Functor m, Monad m) => HSX.EmbedAsAttr (URLT url m) (Attr String String) where
    asAttr (n := str)  = asAttr $ MkAttr (toName n, pAttrVal str)

instance (Functor m, Monad m) => HSX.EmbedAsAttr (URLT url m) (Attr String Bool) where
    asAttr (n := True)  = asAttr $ MkAttr (toName n, pAttrVal "true")
    asAttr (n := False) = asAttr $ MkAttr (toName n, pAttrVal "false")

instance (Functor m, Monad m) => HSX.EmbedAsAttr (URLT url m) (Attr String Int) where
    asAttr (n := i)  = asAttr $ MkAttr (toName n, pAttrVal (show i))

instance (Functor m, Monad m) => EmbedAsChild (URLT url m) Char where
    asChild = XMLGenT . return . (:[]) . UChild . pcdata . (:[])

instance (Functor m, Monad m) => EmbedAsChild (URLT url m) String where
    asChild = XMLGenT . return . (:[]) . UChild . pcdata

instance (Functor m, Monad m) => EmbedAsChild (URLT url m) XML where
    asChild = XMLGenT . return . (:[]) . UChild

instance (Functor m, Monad m) => EmbedAsChild (URLT url m) () where
  asChild () = return []

instance (Functor m, Monad m) => AppendChild (URLT url m) XML where
...
instance (Functor m, Monad m) => SetAttr (URLT url m) XML where
 setAll xml hats = do
        attrs <- hats
        case xml of
         CDATA _ _       -> return xml
...
instance (Functor m, Monad m) => XMLGenerator (URLT url m)
